C'est noté. Voici les modifications pour **remplacer le "Répertoire Staff" par "Annuaire"** et connecter cette liste directement aux données du Google Sheet (Nom, Prénom, Numéro, Poste) comme demandé.

### 1. Fichier `app/api/route.js` (Back-End)

Je modifie la fonction `getInitData` pour qu'elle ne renvoie pas seulement la liste des noms pour le login, mais aussi la liste complète structurée (Annuaire) avec les colonnes B, C, D et L du Sheet.
...
### 2. Fichier `app/page.js` (Front-End)

Ici, j'ai mis à jour le chargement initial (`initApp`) pour remplir la liste de l'annuaire avec les données venant du back-end, et j'ai remplacé le titre "Répertoire Staff" par "Annuaire".
